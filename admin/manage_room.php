...
<div class="container-fluid">
	<form action="" id="manage-room">
		...
		<!-- <div class="form-group">
			...
			<select name="faculty_id" id="faculty" class="custom-select custom-select-sm">
				<option selected disabled hidden>Select Faculty</option>
				<?php
				$qry = $conn->query("SELECT * FROM faculty_list where id not in (SELECT faculty_id FROM room_list where faculty_id is not null)");
				while ($row = $qry->fetch_assoc()) : ?>
					<option value="<?= $row['id'] ?>" <?= isset($faculty_id) && $faculty_id == $row['id'] ? 'selected' : '' ?>><?= ucwords($row['firstname'] . ' ' . $row['lastname']) ?></option>
				<?php endwhile; ?>
			</select>
		</div> -->
	</form>
